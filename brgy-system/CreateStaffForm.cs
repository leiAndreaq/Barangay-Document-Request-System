...
namespace brgy_system
{
    public partial class CreateStaffForm : Form
    {
        ...
        private void InsertStaffIntoDatabase(string firstName, string middleName, string lastName, string gender, int age, string contactNumber, string purokNumber, string phase, string package, string block, string religion, string civilStatus)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                {
                    conn.Open();
                    string query = "INSERT INTO Staff (FirstName, MiddleName, LastName, Gender, Age, ContactNumber, PurokNumber, Phase, Package, Block, Religion, CivilStatus) " +
                                   "VALUES (@FirstName, @MiddleName, @LastName, @Gender, @Age, @ContactNumber, @PurokNumber, @Phase, @Package, @Block, @Religion, @CivilStatus)";
        ...
        private void btnSave_Click_1(object sender, EventArgs e)
        {
            try
            {
                if (ValidateFields())
                {
                    ...
                    InsertStaffIntoDatabase(firstName, middleName, lastName, gender, age, contactNumber, purokNumber, phase, package, block, religion, civilStatus); // Pass civilStatus as a parameter
